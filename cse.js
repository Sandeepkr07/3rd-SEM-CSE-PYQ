...
const links = {
  ...
  '2020': {
    'Mathematics III': 'https://drive.google.com/drive/folders/1XhrlEOOUfMMzndFLSsVRFMkoWijT32eh',
...
function filterYear(selectedYear) {
  ...
  const activeYears = selectedYear === "all" ? Object.keys(links) : [selectedYear];

  activeYears.forEach((year) => {
    subjects.forEach((subject) => {
      const link = links[year]?.[subject];
      if (link) {
        const card = document.createElement("div");
        card.className = "card";
        card.innerHTML = `
          <h3>${subject} (${year})</h3>
          <a href="${link}" target="_blank" class="btn">📂 Open Drive Folder</a>
        `;
